...
select *,rank() over(order by score desc) as RANK_SCORE from student_performance;

##Question 2 : Show each student's score and the previous student’s score (based on score order).
 
  SELECT STUDENT_ID,NAME,SCORE,
  LAG(SCORE) OVER(ORDER BY SCORE DESC) 
  AS PREVIOUS_SCORE 
...
## Question 5 : Assign students into 4 performance groups using NTILE().
        ...
        SELECT NTILE(4) OVER(ORDER BY SCORE DESC ) AS PERF_GROUPS,NAME,ATTENDANCE,SCORE  FROM student_performance;
        
## Question 6 : For each course, assign a row number based on attendance (highest first).

   SELECT  ROW_NUMBER() OVER(ORDER BY ATTENDANCE) AS SNO_ ,NAME,ATTENDANCE FROM STUDENT_PERFORMANCE;
...
SELECT NAME,ATTENDANCE,JOIN_DATE,
DATEDIFF("2025-01-01",join_date) 
as num_of_days 
from STUDENT_PERFORMANCE;

## Question 8 : Format join_date as “Month Year” (e.g., “June 2023”).

 select * , date_format(join_date,"%m %y") as format_date  from STUDENT_PERFORMANCE;
 
 ## Question 9 : Replace the city ‘Mumbai’ with ‘MUM’ for display purposes

  select STUDENT_ID,NAME,CITY ,
  CASE WHEN
  CITY = "MUMBAI" THEN "MUM"
  ELSE CITY
  END AS CITY
  FROM STUDENT_PERFORMANCE;
  ...
  ## Question 10 : For each course, find the highest score using FIRST_VALUE()\
   ...
   SELECT COURSE,SCORE
   ,FIRST_VALUE(SCORE) OVER(PARTITION BY COURSE ORDER BY SCORE DESC) HIGHEST_COURSE_WISE
   FROM STUDENT_PERFORMANCE;
